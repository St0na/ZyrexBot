  ...
  if (!time) {
    if (reason) {
      if (
        !sdb
          .get("mute")
          .find({ guild: message.guild.id, user: kisi.id })
          .value()
      ) {
        let obj12 = {
          guild: msg.guild.id,
          ...
          user: kisi.id,
          user_name: kisi.user.username,
          staff: msg.author.id,
          staff_username: message.author.username,
        ...
        sdb
          .get("mute")
          .find({ guild: msg.guild.id, user: kisi.id })
          .assign(obj12)
          .write();
      }
      if (!kisi.roles.cache.has(muterole2.id))
        await kisi.roles.add(muterole2.id);
      message.channel.send(
        `${kisi} **SINIRSIZ** Şekilde Susturuldu!\nNedeni: **${reason}**\nYetkili: **${message.author}**`
      );
    } else {
      if (
        !sdb
    ...
    if (reason) {
      ...
      if (!kisi.roles.cache.has(muterole2.id))
        await kisi.roles.add(muterole2.id);
      message.channel.send(
        `${kisi} **${time}** Süresince Şekilde Susturuldu!\nNedeni: **${reason}**\nYetkili: **${message.author}**`
      );
      sdb.read();
      let bitiszamani = sdb
        .get("mute")
          ...
          if (bitiszamani <= Date.now()) {
            ...
            if (kisi.roles.cache.find(r => r.id === muterole2.id)) {
              ...
              sdb
                ...
                .write();
              message.channel.send(
                `${kisi} Susturulma Süresi Dolduğu İçin Susturulması Kaldırılmıştır.`
              );
            }
          }
        }, 6000);
